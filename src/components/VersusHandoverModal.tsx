...
export const VersusHandoverModal: React.FC<VersusHandoverModalProps> = ({
  ...
  const nextTier = isP1 ? versus.p1NextTiers[0] : versus.p2NextTiers[0];
  const fruitInfo = FRUIT_CATALOG[nextTier - 1] || FRUIT_CATALOG[0];
  const saltCharges = isP1 ? versus.p1SaltCharges : versus.p2SaltCharges;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/80 backdrop-blur-md animate-fade-in text-[#EDE2D4]">
      ...
      >
        ...
        <button
          ...
          className="w-full py-3.5 px-4 rounded-2xl text-white font-black text-base transition-all shadow-xl active:scale-98 cursor-pointer flex items-center justify-center gap-2"
          style={{
